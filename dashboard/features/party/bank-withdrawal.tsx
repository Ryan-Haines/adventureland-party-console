...
export function useBankWithdrawal(post: PartyConsoleModel["post"], notice: (message: string) => void) {
  ...
  async function submit(request: Request, confirmed = false) {
    ...
    try {
      ...
      setPending(null);
      notice(confirmed ? "Auto-bank mark removed and withdrawal queued" : "Withdrawal mark updated");
    } catch (error) {
      if (error instanceof PartyActionError && error.details.code === "auto_bank_confirmation_required") setPending(request);
      else notice(error instanceof Error ? error.message : "Withdrawal failed");
    } finally {
  ...
  function withdraw(character: string | null | undefined, pack: string, entry: InventoryEntry, markAll = false) {
    if (!character) return notice("No merchant is configured");
    if (pending) return;
    void submit({ character, type: "withdraw", pack, slot: entry.slot, item: { ...entry.item }, markAll });
  }
  const confirmation = <Dialog open={!!pending} onOpenChange={open => { if (!open && !busy) setPending(null); }}>
    <DialogContent showCloseButton={false} className="border border-emerald-800 bg-[#091614] text-emerald-50">
      ...
      <DialogFooter>
        <Button disabled={busy} onClick={() => setPending(null)} className="border border-slate-500 bg-[#101c1a] text-slate-100 hover:bg-slate-700 hover:text-white">Cancel</Button>
