...
export function LogSidebar({state,width,onWidth,onClose}:{state:PartyState;width:number;onWidth:(n:number)=>void;onClose:()=>void}){
  ...
  const game=Object.entries(query.data?.gameLogs||{}).flatMap(([name,events])=>events.map(e=>({...e,category:classifyGameLog(e.message),name,source:'game',key:name+e.session+e.seq})));
  const combat=Object.entries(query.data?.combatLogs||{}).flatMap(([name,events])=>events.map((e,i)=>({at:e.at,message:e.message,name,source:'combat',category:e.type,key:`combat:${name}:${e.at}:${i}`,color:''})));
  const merchant=(query.data?.merchantActivity||[]).map((e,i)=>({at:e.at,message:e.message,name:state.merchantCharacter||'',source:'merchant',category:e.level,key:`merchant:${e.at}:${i}`,color:''}));
  const anniversary=(state.anniversary?.activity||[]).map((e,i)=>({at:e.at,message:e.message,name:'',source:'anniversary',category:e.level,key:`anniversary:${e.at}:${i}`,color:''}));
  const entries=(tab==='game'?game:[...combat,...merchant,...anniversary]).filter(e=>(character==='all'||e.name===character)&&(tab==='game'?showGameLog(e.category||'other',filters):source==='all'||source===e.source)).sort((a,b)=>a.at-b.at).slice(-1000);
  useEffect(()=>{if(following.current && scroller.current)scroller.current.scrollTop=scroller.current.scrollHeight;},[query.data,tab,character,source,filters]);
  const names=Array.from(new Set([...Object.keys(state.characters),...Object.keys(query.data?.gameLogs||{}),...(state.bankbois||[]).map(b=>b.name)]));
  ...
  return <aside aria-label="Live logs" className="fixed bottom-0 right-0 top-0 z-40 flex flex-col border-l border-slate-500 bg-slate-950 text-slate-100 shadow-xl" style={{width,maxWidth:'75vw'}}>
    ...
    <div ref={scroller} onScroll={e=>{const el=e.currentTarget;following.current=el.scrollHeight-el.scrollTop-el.clientHeight<32;}} className="min-h-0 flex-1 space-y-2 overflow-auto px-3 pb-3 font-mono text-xs">{entries.map(e=><p key={e.key} className="break-words"><span className="text-slate-500">{new Date(e.at).toLocaleTimeString()} {e.name ? `[${e.name}]` : `[${e.source}]`} </span><span style={{color: /^#[0-9a-f]{3,8}$/i.test(e.color)?e.color:undefined}} className={e.category==='errors'||e.category==='error'?'text-rose-300':'text-slate-100'}>{e.message}</span></p>)}{!entries.length&&<p className="text-slate-400">No matching logs yet.</p>}</div>
  </aside>;
}
