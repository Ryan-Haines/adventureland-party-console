...
test(`${implementation} coordinator handles first and repeated heartbeats without a current combat target`,async()=>{
 ...
 const contracts=require('./fixtures/heartbeat-contracts.json').map(value=>({...value,merchantService:null,leader:'P',desiredPartyMembers:['P'],leaderLocation:{map:'main',x:0,y:0},monsterFocus:[],scatterEpoch:0}));
