...
test('browser timezone formats the event instant without changing its countdown', () => {
  ...
  const body = source.slice(source.indexOf('export function eventTimeLabel('),source.indexOf('export function EventSelectionControl(')).replace('export function','function');
