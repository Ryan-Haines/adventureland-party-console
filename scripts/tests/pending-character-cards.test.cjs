...
test('pending cards show Steam progress without inventing telemetry or offering game commands',()=>{
 ...
 const model={chars:[{name:'Ready'}],state:{characterConnections:[
  {name:'Ready',status:'connected'}, {name:'Primary',status:'code',primary:true}, {name:'Mage',status:'loading',delayed:true,error:'Retrying connection'}
 ...
 assert.match(text,/CODE active/);assert.match(text,/Loading in Steam/);assert.match(text,/Taking longer/);assert.match(text,/Retrying connection/);
