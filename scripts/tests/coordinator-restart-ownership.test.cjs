...
test('restart retires its own service launcher and leaves an unrelated parent alone',()=>{
 const file=path.resolve('scripts/start-console.ps1').replaceAll("'","''");
 const code=`
 ...
 function Get-CimInstance {
  ...
  switch ($Filter) {
   ...
   'ProcessId=10' {return [pscustomobject]@{ProcessId=10;Name='pwsh.exe';CommandLine=('pwsh -File scripts/'+$script:starterName)}}
