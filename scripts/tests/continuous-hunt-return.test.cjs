...
for(const map of ['mansion','winterland'])for(const disableTown of [false,true])test('three managed runners return from '+map+' to Daisy on one itinerary; disableTown='+disableTown,async()=>{
 ...
 const p={nextCommandId:10,commands:{},navigationIntents:{},combatLogs:{},eventReturn:null,
  statuses:Object.fromEntries(names.map(n=>[n,{name:n,seenAt:now,hp:100,map,x:1000,y:0,speed:57,server:'USII',huntReturnProtocol:1,convoyProtocol:4,combatSelection:{runtimeId:n}}])),
  activeConvoy:{id:'return',epoch:1,routeProtocol:4,phase:'assemble',leader:'L',participants:names,completed:[],slowestSpeed:57,
   rally:{map,x:1000,y:0},location:{map:'main',x:120,y:0},purpose:'monster-hunt',returnRouting:true,disableTown,combatHandoffAllowed:false}};
 ...
 const runners=names.map(n=>{
  const r=runtime({plan:async b=>{await settle();requests.push([n,b.town]);return {...b,ms:1,plot:[
  ...
  vm.runInContext(['defendPartyHit','interruptConvoyForDefense'].map(name=>namedFunction(source,name)).join('\n'),c);
 ...
 for(let i=0;i<220 && p.activeConvoy;i++){
  ...
  runners.forEach((r,j)=>{
   ...
   if(local) { c.defendPartyHit({id:names[j],hid:'passing-armadillo'});assert.equal(c.convoyTraveling,local,'passing retaliation must retain return ownership'); }
 ...
 assert.equal(convoy.routeVersion,1);assert.equal(convoy.epoch,1);
 assert.equal(p.nextCommandId,13,'no per-leg command replacements');
 assert.deepEqual(requests,disableTown?[['L',false]]:[['L',false],['L',true]]);
 for(const r of runners){assert.equal(r.context.character.map,'main');assert.equal(r.context.character.x,120);
  assert.equal(r.calls.filter(c=>c[0]==='cruise'&&c[1]===57).length,1);
